...
const TIER_CONFIG = Object.fromEntries(TIERS.map(t => [t.key, t]));
...
export default function Tierlist() {
  ...
  const [editEntry, setEditEntry] = useState(null); // for edit modal
  ...
  return (
    <div className="tierlist-page">
      ...
      <div className="page-top-bar">
        <h1 className="section-title">Tier List <span>({totalCount})</span></h1>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", marginTop: 80, color: "var(--text-muted)" }}>
          ...
          <p style={{ fontSize: "0.875rem" }}>Open an anime and click <strong>"Add to Tier List"</strong> to rank it.</p>
        </div>
      ) : (
            ...
            return (
              <div key={tier.key} className="tier-section">
                <div className={`tier-label ${tier.key}`}>
                  {tier.label}
                  <span style={{ fontSize: "0.8rem", fontWeight: 400, opacity: 0.6, marginLeft: 10 }}>{tier.desc}</span>
      ...
      {editEntry && (
        <div className="tier-modal-overlay" onClick={e => e.target === e.currentTarget && setEditEntry(null)}>
          <div className="tier-modal">
            <button className="tier-modal-close" onClick={() => setEditEntry(null)}>✕</button>
            ...
            {editEntry.comment && !editComment && (
              <p style={{ color: "var(--text-muted)", fontSize: "0.8rem", marginTop: 6, fontStyle: "italic" }}>
                Saved comment: "{editEntry.comment}"
              </p>
            )}
