...
const TIERS = [
  { key: "peak",  label: "PEAK",  color: "#ff4500" },
  { key: "great", label: "GREAT", color: "#ff8c00" },
  { key: "good",  label: "GOOD",  color: "#32cd32" },
  { key: "mid",   label: "MID",   color: "#1e90ff" },
  { key: "bad",   label: "BAD",   color: "#9370db" },
];
...
export default function AnimeDetail() {
  ...
  async function checkFavorite() {
    try {
      const res = await fetch(`${API_BASE}/favorites`, { headers: authHeader(), credentials: "omit" });
      if (!res.ok) return;
  ...
  async function toggleFavorite() {
    if (!anime) return;
    if (isFavorite) {
      await fetch(`${API_BASE}/favorites/${anime.mal_id}`, { method: "DELETE", headers: authHeader(), credentials: "omit" });
      setIsFavorite(false);
    } else {
  ...
  async function checkWatchlist() {
    try {
      const res = await fetch(`${API_BASE}/watchlist`, { headers: authHeader(), credentials: "omit" });
      if (!res.ok) return;
      const data = await res.json();
  ...
  async function checkTierlist() {
    try {
      const res = await fetch(`${API_BASE}/tierlist`, { headers: authHeader(), credentials: "omit" });
      if (!res.ok) return;
      const data = await res.json();
      if (Array.isArray(data)) {
  ...
  if (!anime) return (
    <div>
      <Navbar />
      <p className="loading" style={{ marginTop: 80 }}>Loading</p>
    </div>
  );

  const tierInfo = existingTier ? TIER_MAP[existingTier.tier] : null;
        ...
        <div className="detail-layout">
          ...
          <div className="detail-main">
            ...
            <div className="action-bar">
              ...
              >
                {tierInfo ? `${tierInfo.label} Tier` : "Add to Tier List"}
              </button>
            </div>
            ...
            <div className="meta">
              ...
              {anime.aired?.from && <span>{anime.aired.from.slice(0, 4)}</span>}
      ...
      {showTierModal && (
        <div className="tier-modal-overlay" onClick={e => e.target === e.currentTarget && setShowTierModal(false)}>
          <div className="tier-modal">
            <button className="tier-modal-close" onClick={() => setShowTierModal(false)}>x</button>
            ...
            <div className="tier-select-row">
              {TIERS.map(t => (
                <button
                  key={t.key}
                  className={`tier-select-btn ${t.key}-btn${selectedTier === t.key ? " selected" : ""}`}
            ...
            <button className="tier-modal-save" onClick={saveTierlist} disabled={!selectedTier}>
